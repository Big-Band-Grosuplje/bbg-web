...
- **IP-jev obiskovalcev ne pošiljamo tretjim osebam, ki niso nujne za delovanje strani.** Izjeme, razkrite v `/zasebnost`: gostitelj Vercel (serviranje + Web Analytics, brezpiškotno), YouTube nocookie ob kliku na posnetek, Brevo in Web3Forms ob oddaji obrazca
  ...
  - ⚠️ **Odstopanje pravila od stanja, izmerjeno 4. 9. 2026:** sličice posnetkov so vezane na `i.ytimg.com` (`src/lib/mediji.ts`), zato Google dobi IP **brez klika** — 8 zahtevkov že ob nalaganju `/multimedija` in `/en/media`, 3 na naslovnici po drsenju do izseka (`loading="lazy"`). Pravilo zgoraj in besedilo `/zasebnost` („Prikaže se le sličica, predvajalnik pa nastane šele, ko klikneš gumb za predvajanje") pokrivata samo predvajalnik, ne sličice. Do odprave — prenos sličic ob buildu in samo-gostovanje — je izjema **širša, kot je zapisana**
