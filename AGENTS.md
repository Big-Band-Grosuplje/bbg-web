...
- **IP-jev obiskovalcev ne pošiljamo tretjim osebam, ki niso nujne za delovanje strani.** Izjeme, razkrite v `/zasebnost`: gostitelj Vercel (serviranje + Web Analytics, brezpiškotno), YouTube nocookie šele ob kliku na posnetek, Brevo in Web3Forms ob oddaji obrazca
  ...
  - **Sličice posnetkov so samo-gostovane** v `src/assets/mediji-thumbs/<id>.jpg` in v gitu. Prenese jih `npm run slicice` — **`npm run build` jih namenoma ne prenaša**, da produkcijski build ne kliče YouTuba (postopek je opisan v razdelku Ročni postopki). Ob dodajanju posnetka v `mediji.json` torej zaženi `npm run slicice`; brez datoteke build pade z napako iz `slicicaZa` in ne izpiše tihe prazne slike. Ali ima sličica črna pasova (hqdefault je 4:3), se ugotovi iz razmerja stranic datoteke in se v podatke ne zapisuje. Odpravljeno 4. 9. 2026; prej je bila sličica vezana na `i.ytimg.com` in je Google dobil IP brez klika
...
- **Vsak obrazec v sekciji Kontakt ima svoje sidro**: `#organizatorji` in `#pridruzi-se`, `#delavnica` pa je na naslovu panela Pridruži se in dodatno predizbere področje. Eno samo sidro `#kontakt` je bilo napaka, ki se je pokazala šele na ozkem zaslonu: panela sta tam v enem stolpcu in obiskovalec je pristal na prvem (Za organizatorje), Pridruži se pa je bil ~740 px nižje, izven zaslona. Novih praznih elementov za sidra ne dodajaj — panela sta celici mreže in bi razdrla dvostolpčno postavitev
- **Sidra ne smejo pristati pod lepljivo navigacijo**: `html { scroll-padding-top: 88px }` v `brand.css` velja za vsa sidra na vseh straneh. `scroll-margin-top` po posameznih elementih ne dodajaj — razleteti se more. Pri zadnjem bloku pred nogo cilj obvisi nižje od odmika, ker stran nima več prostora za drsenje; to ni napaka
...
- **Arhiv dogodkov je v DVEH datotekah**: `src/data/arhiv-uvoz.json` piše samo `npm run arhiv`, `src/data/arhiv-rocno.json` samo človek. Ročnega vpiši nikoli v uvoženo — naslednji zagon ga prepiše. Ključ je `id` (`YYYY-MM-DD-<kraj>`), dovoljena polja `naziv`, `opis`, `zasedba`, `prizorisce`, `ura`; `kraj` med njimi ni, ker je `id` izpeljan iz njega. Ročni vnos brez zapisa v uvozu **podre gradnjo** — vpisana vrednost, ki je na strani ni, je tiha izguba dela
- **Pretekli dogodki iz `koncerti.json` se ob gradnji zlijejo v arhiv** (`src/lib/arhiv.ts`), ker jih arhivski vir ne pozna in bi po preteku izginili z vseh strani. Zlivaj samo tam — vpis v `arhiv-uvoz.json` bi naslednji `npm run arhiv` pobrisal. Merilo prehoda je **isti** `jePrihajajoc(datumKonecIso ?? datumIso)` kot pri napovedniku in velja **tudi za arhivske vnose** (uvoz vsebuje tudi še nenastopljene datume). Ob ujemanju po datumu in kraju prevlada `koncerti.json`; vnos iz njega je povezan na svojo podstran, **zaprt dogodek pa ne** — njegova podstran je `noindex` prav zato, da je nepovabljeni ne najde
- **Arhivski vnos ima `vloga`** (`izvajalec` privzeto, `organizator` izjema): orkester je nekatere dogodke pripravil, ne odigral. Pri `organizator` je **zasedba prazna** — to je pravilna vrednost in ne manjkajoča, nadomestna bi bila laž; vnos ne šteje pri izbirah zasedbe in namesto značke zasedbe nosi značko „organizacija". Vloge **iz besedila ni mogoče izpeljati** („Nastopil HGM Jazz Orchestra" je ista oblika kot „Nastopil je tudi …"), zato jo vpiše človek v `arhiv-rocno.json` in uvoz je nikoli ne zapiše. Ročno se vpiše **samo vloga** — prazno zasedbo nastavi združitev
- **Zasedba arhivskega vnosa je MNOŽICA** (`["big-band"]`, `["combo"]`, `["big-band", "combo"]`), ker sta na istem dogodku lahko nastopili obe. Filter in števci se ujemajo po **vsebovanosti**, zato vsota izbir lahko preseže število vseh vnosov; stran to pove v vrstici pod gumbi, ki se izpiše samo, kadar tak dogodek res obstaja
- ⚠️ **Zasedbo ima zapisanih 44 od 291 vnosov, pri ostalih je izpeljana** po pravilu „kjer zapis ne omenja comba ali male zasedbe, je igral big band". Izpeljavo dela `npm run arhiv` in ne izris, vsak vnos pa nosi `zasedbaVir` (`zapisano` | `izpeljano` | `rocno`). **Polja ne odstranjuj in izpeljave ne prestavljaj v komponento** — brez njega bi bilo 210 vnosov videti kot zabeležen podatek, čeprav so sklep iz odsotnosti besede. Dvojne zasedbe iz besedila NE izpeljujemo: „Combo zasedbe Big Banda Grosuplje" je rodilnik in ne dva nastopa
...
- `prijava: true` pomeni prost vstop z **obvezno prijavo**: ob znački se izpiše opomba in gumb, ki pelje na sidro `#delavnica` — to pripelje na obrazec Pridruži se tudi brez JS, skripta pa ob tem sidru predizbere možnost v izbirniku, zamenja zadevo sporočila in sprosti obveznost polja Sporočilo (ime in e-pošta ostaneta obvezna). Query `?prijava=delavnica` ostaja podprt zaradi že objavljenih povezav
...
## Ročni postopki
Vsi tečejo **zunaj builda**, ker zahtevajo mrežo, velike izvirnike ali gradivo, ki v git ne gre. Produkcijski build ne sme biti odvisen od nobenega od njih.

- **`npm run slicice`** — prenese sličice posnetkov z YouTuba v `src/assets/mediji-thumbs/<id>.jpg` (maxresdefault, sicer hqdefault; obstoječih datotek ne prepiše). Zaženi ob dodajanju posnetka v `mediji.json`. Razlog, da tega ne dela build, je v razdelku Tehnična pravila
- **`npm run arhiv`** — pretvori `_vhod/zapisi.json` v `src/data/arhiv-uvoz.json` (glej `docs/uvoz-zgodovine.md`). **Piše samo to datoteko in jo vsakič prepiše v celoti.** Ročne dopolnitve zato živijo v ločeni `src/data/arhiv-rocno.json`, ki se je skripta ne dotakne; združi ju `src/lib/arhiv.ts` ob gradnji, ročna vrednost prevlada. Ob razhajanju med virom in ročnim zapisom skripta obe vrednosti izpiše in **ne odloči sama**. Zapiše tudi predloge za program in prireditev v `_vhod/predlogi.md` (registra še ni, polj v podatkih ni) — in te datoteke **ne prepiše, dokler obstaja**, ker so v njej lahko že vpisane odločitve. Enako velja za `_vhod/preveri-opise.md`: seznam zapisov, pri katerih je vir isti opis uporabil pri več dogodkih in torej beleži delovno oznako, ne dogodka
- **`npm run slike [ciljna-mapa]`** — obdela fotografije iz `_vhod/` (mapa ni v gitu): daljša stranica 2000 px, JPEG 85, brez EXIF, EXIF orientacija upoštevana pred izbrisom. Privzeta ciljna mapa je `src/assets/foto/zgodovina`, drugo podaj kot argument. Obstoječih datotek v cilju ne prepiše
  - Ime izhodne datoteke nastane iz imena vhodne (slug). Če hočeš uredniško ime, **preimenuj datoteko v `_vhod/` pred zagonom**
  - ⚠️ Skripta preskoči samo datoteke, katerih **slug** že obstaja v cilju. Če je slika v cilju pod drugim, uredniškim imenom, je ponovni zagon ne prepozna in naredi še eno kopijo pod slug imenom. `_vhod/` je zato nabiralnik: po prenosu izvirnike odnesi iz njega
